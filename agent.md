...
78 tests total. `@anthropic-ai/sdk` is fully mocked in every committed test — the only real-API calls in this repo happen via throwaway `_smoke_test.mts` scripts, always deleted after use.
...
**All of Level 1 MVP scope from the PRP is complete (tasks 1.0–9.0).** The full Stage 1-5 pipeline works end-to-end (brief → Intake → clarification → Triage → specialist review → Deliverables + Services Document), plus three ad-hoc UI additions (step-console restructure, project summary bar, checklist-in-sidebar), the Outputs Library + Version History, ad-hoc Knowledge Upload, and the project-scoped Chatbot with cross-project isolation proven by a real-DB integration test. Task 9.0 added: a responsive pass (mobile/tablet/desktop, all clean), an accessibility pass (fixed 4 real unlabeled-input gaps, added `aria-live` to the chatbot, verified WCAG AA contrast by hand), a genuine end-to-end integration test that calls the real Server Actions in sequence, a secrets audit (clean, plus caught and fixed a real forgotten-placeholder `NEXTAUTH_SECRET`), and deployment to Vercel with the live URL verified end-to-end (login, real data, real chatbot answer, all against the real Neon database). Typecheck, lint, full test suite (78 tests), and production build all pass, locally and on Vercel. Two known non-blocking items from this session: (1) `.env.local` was briefly displayed in full in the chat transcript during the secrets audit — the user was informed and chose not to rotate the exposed credentials; (2) the Browser-preview tool's click/keypress dispatch broke mid-session (tool-side, not app-side) — verification for the remainder of 9.0 relied on screenshots, DOM inspection, and real `form.requestSubmit()` calls instead. Nothing left for Level 1 MVP; Level 2/3 (Stages 6-10, estimation, commercials, capability briefing, status monitoring) would be the next scope if this pilot continues.
