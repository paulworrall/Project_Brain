...
Next.js 16 (App Router, TypeScript, Turbopack) with Tailwind CSS v4, Prisma 7 (Postgres/Neon, driver-adapter-based), and NextAuth v5 (beta) for email/password auth. All Level 1 MVP scope (Stages 1-5, Knowledge Upload, Chatbot, polish/QA/deploy) is complete, plus post-MVP additions: a Phase-grouped Stage Tracker (`src/lib/phases.ts`), full Client/Workstream taxonomy navigation + global search, Phase 1 reworked into a progressively-disclosed fluid workspace, commercial document management generalized into one shared "document has many Versions, exactly one current" pattern used identically by Master Service Agreements, Rate Cards, and a SOW Template Library, and — **newest** — the Client detail page's MSA and Rate Cards sections now use that same "library" read-list UX SOW Templates already had (compact rows + "Manage in library →" link to a dedicated management page), instead of full inline version-history panels. Phases 2-3 (Stages 5-9) still render as the original step-card list; Stage 8 ("Commercials & SOW") hosts a real SOW-Template-selection panel instead of a pure placeholder. Stage 6/7/9/10 remain inert placeholders. The Triage/Clarification agents and Document/DocumentVersion model are untouched.
...
| `src/app/(dashboard)/clients/[clientId]/page.tsx` + `actions.ts` | Client Detail — Workstreams; Contracts & Rates now renders `ClientMasterServiceAgreementSummary` + `ClientRateCardsSummary` (compact library-style read lists, **not** the full `VersionHistory` panels — those moved to the dedicated library pages below); `ClientSowTemplatesSection` (unchanged: baseline + this Client's SOW Template variants, plus a quick "Add variant" action). `actions.ts` still holds all MSA/Rate Card version actions, used now by the library pages instead |
| `src/app/(dashboard)/master-service-agreements/page.tsx` | **New** — the MSA library: every Client (including ones with none yet), each rendering the full `MasterServiceAgreementsPanel`/`VersionHistory`. Mirrors `/sow-templates`'s structure |
| `src/app/(dashboard)/rate-cards/page.tsx` | **New** — the Rate Cards library: every Client's named Rate Cards, grouped by Client, each its own `VersionHistory` via `RateCardsPanel`. Creating a brand-new named Rate Card is **not** available here — only on the Client page (`ClientRateCardsSummary`) |
| `src/app/(dashboard)/sow-templates/page.tsx` + `actions.ts` | The SOW Templates library: the baseline GLOBAL template plus every client-specific variant grouped by Client, each as its own `VersionHistory` (via the `SOWTemplateVersionHistory` Client Component wrapper — see note below). `actions.ts`: `getSOWTemplatesForClientAction` (Client-scoped lookup — baseline + this Client's variants only, same isolation pattern as Rate Cards), `createClientSpecificSOWTemplateAction`, `uploadSOWTemplateVersionAction`, `revertSOWTemplateVersionAction` |
...
| `src/components/layout/Sidebar.tsx` | Nav: "All Projects", **new** "Master Service Agreements" (`/master-service-agreements`), **new** "Rate Cards" (`/rate-cards`), "SOW Templates" (`/sow-templates`) |
...
| `src/components/features/MasterServiceAgreementsPanel.tsx` | A thin wrapper around one `VersionHistory` (an MSA is a single unnamed document per Client). Used only on `/master-service-agreements` now, once per Client |
| `src/components/features/RateCardsPanel.tsx` | **Changed this session**: no longer has any creation UI — now purely a list of one `VersionHistory` per named Rate Card document that already exists (used on `/rate-cards`, once per Client). Also exports `EffectiveDateFields` (the effective-from/to date inputs), reused by `ClientRateCardsSummary`'s create form so the two aren't duplicated |
| `src/components/features/LibrarySummaryList.tsx` | **New** — the shared read-only "library" card: header + "Manage in library →" link + rows (name, optional `(tag)`, right-aligned filename or "Not yet uploaded"), rows list scrollable (`max-h-64 overflow-y-auto`) if long, optional `children` slot for a quick-add form. Generalized out of what `ClientSowTemplatesSection.tsx` had hand-rolled; now used by all three Client-page commercial-document summaries |
| `src/components/features/ClientSowTemplatesSection.tsx` | Refactored to build its rows and pass them into `LibrarySummaryList`, keeping only its own "Add [Client]-specific variant" quick-add form as `children`. Behavior/markup otherwise unchanged from the prior session |
| `src/components/features/ClientMasterServiceAgreementSummary.tsx` | **New** — Client-page read view of the MSA via `LibrarySummaryList` (0 or 1 row, tagged "active"), linking to `/master-service-agreements`. No quick-add — an MSA has no "create a new named document" step the way Rate Cards/SOW variants do |
| `src/components/features/ClientRateCardsSummary.tsx` | **New** — Client-page read view of this Client's Rate Cards via `LibrarySummaryList` (each row tagged "current"), linking to `/rate-cards`, plus the "Add rate card" create form moved out of `RateCardsPanel.tsx` (mirrors exactly where SOW's "Add variant" already lived: on the Client page, not the library page) |
| `src/components/features/StartSowDevelopmentPanel.tsx` | Stage 8's real content: a SOW Template `<select>` (scoped to baseline + the Project's own Client) + save button, wired to `startSowDevelopmentAction`. "Generate SOW" stays an explicit disabled placeholder — that agent doesn't exist yet |
| `src/lib/extractTextFromBuffer.ts` | The file-parse-and-wrap-errors helper, shared by `clients/[clientId]/actions.ts` and `sow-templates/actions.ts` |
...
### Test Files (196 total, all passing)
This session's additions: new `tests/components/{LibrarySummaryList,ClientMasterServiceAgreementSummary,ClientRateCardsSummary}.test.tsx`. `RateCardsPanel.test.tsx` trimmed to drop the creation-flow assertions (moved to `ClientRateCardsSummary.test.tsx`). `ChatPanel.test.tsx` had one pre-existing flaky assertion (synchronous check right after a click, before an async state update flushed) wrapped in `waitFor` — unrelated to this session's feature work, found only because it flaked under the full suite's parallel load. Prior session's additions (`commercial-documents-versioning.test.ts`, `VersionHistory.test.tsx`, `StartSowDevelopmentPanel.test.tsx`, `ClientSowTemplatesSection.test.tsx`, etc.) unchanged. All other test files unchanged.
...
## Deployment
Live URL: `https://project-brain-ten.vercel.app`. Vercel team `MAP` (scope `map25`), project `project-brain`, linked to `github.com/paulworrall/Project_Brain`'s `main` for continuous deployment. The commercial-documents generalization (SOW Template Library, MSA/Rate Card versioning) was pushed and deployed in a prior session; **this session's library-pattern UX extension to MSA/Rate Cards is committed locally but not yet pushed** — pending user review. Redeploy manually: `npx vercel deploy --prod --scope map25`. This session made no schema/data changes — no migration to worry about, purely presentational + one component reorganization.
...
All Level 1 MVP scope plus prior post-MVP additions (Phase Presentation Layer, taxonomy navigation, Phase 1's progressive-disclosure workspace) remain complete and previously verified. **Prior session**: commercial document management generalized across the platform — MSA and Rate Cards migrated (no data loss) into a shared "document has many Versions, exactly one current" pattern via a reusable `VersionHistory` component, plus a new SOW Template Library; a Stage 8 panel lets a PM record which SOW Template a Project will use. **This session**: purely a UX-consistency pass, no schema change. The Client detail page's MSA and Rate Cards sections were too heavy (full inline upload/revert `VersionHistory` panels) compared to how SOW Templates already presented there (a compact read list + "Manage in library →" link). Generalized that read-list shape into `LibrarySummaryList.tsx` and reused it for all three; moved MSA/Rate Card full management onto two new pages (`/master-service-agreements`, `/rate-cards`) mirroring `/sow-templates`'s existing structure; moved Rate Card creation off the (now management-only) `RateCardsPanel` and onto the Client page's new `ClientRateCardsSummary`, exactly where SOW's variant creation already lived. Verified in the browser as ClientEngagement against real data (Coffee's migrated MSA/Rate Card render as proper library rows; Fizzy's empty MSA/Rate Card show the correct empty state without also showing a list; all three "Manage in library" links resolve to distinct, correct destinations; visible focus ring confirmed on those links). Caught and fixed one genuinely ambiguous test query (`getByText` matching both a card's heading and its only row, both reading "Master Service Agreement") and one unrelated pre-existing flaky assertion in `ChatPanel.test.tsx` (timing, not a regression). Full suite (196 tests, up from 183), typecheck, lint, and production build are all clean. This checkpoint's commit is local-only — do not push until the user explicitly reviews and approves.
