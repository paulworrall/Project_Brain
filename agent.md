...
Next.js 16 (App Router, TypeScript, Turbopack) with Tailwind CSS v4, Prisma 7 (Postgres/Neon, driver-adapter-based), and NextAuth v5 (beta) for email/password auth. The full data schema and three live AI agent pipelines (Intake, Clarification Extraction + Triage, Specialist Review Extraction) are built and verified end-to-end in the browser against the real Claude API and real Neon database. The project page is a single-view "workflow console" — a numbered, expandable step list (all 10 pipeline stages) beside a chat/knowledge-upload sidebar. Stages 1-5 of the pipeline work fully; Stage 6 is unlocked (ready for its own task) but not yet built; Stages 7-10, the chatbot, and knowledge upload render as inert placeholders.
...
| `src/components/features/DeliverablesServicesDocumentView.tsx` | Renders the Deliverables + Services Document — Deliverables list, Services table (5 fixed rows via `SERVICE_ROWS` + the 6th "Other" row using `EditableOtherLabel`), Open Questions/Risks, Outstanding Gaps Carried Forward (warning-styled Card) |
...
| `src/types/deliverables-services.ts` | `DeliverablesServicesDocumentSchema`/`DeliverablesServicesDocument` — deliverables, a **fixed object** (not an array) of the 6 Services capability rows (`experienceCreative`/`business`/`architecture`/`techAndData`/`orchestration` each `{involvement}`, plus `other` with an extra free-text `label`), `openQuestionsRisks`, `outstandingGapsCarriedForward`. Modeled as a fixed object because structured outputs don't enforce array length server-side. Also exports `SERVICE_ROWS`, the 5-row `{key, label}` list the UI maps over for the fixed capabilities |
...
| `prisma/schema.prisma` | Full schema: taxonomy, Stage/ProjectStageStatus, Document/DocumentVersion (JSON content, typed in app code per `DocumentType` — the Zod schemas in `src/types/intake.ts`, `src/types/triage.ts`, `src/types/deliverables-services.ts`), ChecklistItem, TouchpointNote, KnowledgeItem, User. `Project` also carries nullable summary fields: `jobCode`, `kickOffDate`, `targetCompletionDate`, `projectManagerId` (FK → `User`, `onDelete: SetNull`; reverse relation `User.projectsManaged`) — operational metadata, not agent-populated. No schema changes were needed for task 6.0 — Deliverables + Services content lives in the existing `DocumentVersion.content` JSON column |
...
| `tests/services/specialist-review-extraction.test.ts` | 3 — returns parsed document with all six service rows present (including "Not required." and the "Other" label), no-parsed-output error, unexpected-error wrapping | ✅ passing |
...
51 tests total. `@anthropic-ai/sdk` is fully mocked in every agent test per CLAUDE.md — no real API calls in the suite.
...
- **After any `npm install` from WSL, immediately run `npm install` again from native Windows before doing anything else.** Windows/Linux optional-dependency native binaries (`esbuild`, `rolldown`, `@prisma/engines`) do not reliably coexist — a WSL-side install can silently drop the Windows binary Vitest/tsx need. Also persisted in cross-session memory (`prisma_migrate_needs_wsl` memory file) since it corrects an earlier wrong assumption there.

## Current State Summary
Tasks 1.0–6.0 are complete, plus two ad-hoc additions done at the user's request: a project-page restructure (step-console UI, between tasks 4.0 and 5.0) and a project summary bar (Job Code/Kick-off/Target Completion/PM, after task 5.0). The pipeline now works end-to-end through Stage 6: paste/upload a brief → Intake Agent → clarification notes → Clarification Extraction → Triage Agent (Draft Scope Document) → specialist-lead feedback → Specialist Review Extraction (Deliverables + Services Document, with an editable "Other" capability label) → Stage 6 unlocked. Every stage transition, document version, and piece of generated content has been verified in the browser against the real Claude API and real Neon database, not just typechecked. Typecheck, lint, full test suite (51 tests, Anthropic API fully mocked per CLAUDE.md), and production build all pass. Next up: task 7.0 — Outputs Library, Stage Tracker & Version History, which reads across whatever the pipeline has produced so far rather than adding new agent logic.
