 ...
 */
export async function renderEstimateDocumentXlsx(content: EstimateDocumentContent): Promise<Buffer> {
  const { overview, capabilitySections, currency, totalValue } = content;

  const workbook = new ExcelJS.Workbook();
  const sheet = workbook.addWorksheet("Estimate");
  sheet.columns = [{ width: 28 }, { width: 16 }, { width: 16 }, { width: 14 }, { width: 14 }, { width: 16 }];
  ...
  sheet.addRow(["Rate card", `${overview.rateCardName} (version ${overview.rateCardVersionNumber})`]);
  sheet.addRow([]);
  ...
  for (const section of capabilitySections) {
    ...
    const headerRow = sheet.addRow(["Role", "Level", "Rate", "Fee", "Running Total"]);
    ...
    for (const line of section.lineItems) {
      ...
      sheet.addRow([line.role, line.level ?? "—", rateLabel, line.feeSubtotal.toFixed(2), runningTotal.toFixed(2)]);
    }

    const subtotalRow = sheet.addRow([`${capabilityLabel(section.capability)} subtotal: ${section.subtotal.toFixed(2)} ${currency}`]);
